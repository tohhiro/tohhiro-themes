...
function insert_jin_seo_fields($post, $metabox) {
	...
	// canonical設定
    echo '<table style="margin-bottom: 16px;text-align:left;"><tbody>';
    echo '<tr><td style="font-size:13px; font-weight:bold;">canonical設定</td></tr><tr><td></td></tr><tr><td><input name="jin_canonical" class="jin-canonical jin-input" placeholder="" size="50%" value="'.esc_html(get_post_meta($post->ID, 'jin_canonical', true)).'" /></td></tr>';
    echo '</tbody></table>';
	echo '<div style="border-bottom:1px solid #ccc; height:5px; margin-bottom:15px;"></div>';
	// noindex設定
	echo '<table style="margin-bottom: 16px;text-align:left;"><tbody>';
...
function save_jin_seo_fields( $post_id ) {
	...
	$jin_canonical_data = isset($_POST['jin_canonical']) ? $_POST['jin_canonical'] : null;
    if(get_post_meta($post_id, 'jin_canonical') == ""){
        add_post_meta($post_id, 'jin_canonical', $jin_canonical_data, true);
    }elseif($jin_canonical_data != get_post_meta($post_id, 'jin_canonical', true)){
        update_post_meta($post_id, 'jin_canonical', $jin_canonical_data);
    }elseif($jin_canonical_data == ""){
        delete_post_meta($post_id, 'jin_canonical', get_post_meta($post_id, 'jin_canonical', true));
    }
